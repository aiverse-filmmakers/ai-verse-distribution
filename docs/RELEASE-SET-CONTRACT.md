...
The locked native dependency `better-sqlite3@13.0.3` ships its own Linux/macOS/Windows prebuilt binaries and declares `gypfile=false`. Some Windows npm versions still invoke an implicit `node-gyp rebuild` even though the shipped prebuild is the package's intended runtime path. Distribution therefore runs `npm ci --ignore-scripts` for this frozen companion-lock install, then immediately loads `better-sqlite3`, opens an in-memory SQLite database, performs a real create/insert/select cycle, and fails closed if the native binary cannot execute. Data's own TypeScript build is then run explicitly. This avoids an unintended toolchain rebuild without weakening native-runtime verification.
